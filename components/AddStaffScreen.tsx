...
export const AddStaffScreen: React.FC<AddStaffScreenProps> = ({ staffType, businessId, onBack, onNext }) => {
  const [formData, setFormData] = useState({
    ...
    phoneNumber: '',
    dob: '',
    gender: 'Male', // Defaulting to one option to avoid empty state complexity for select
    salaryCycleDate: 1,
  ...
  const isFormValid = 
    ...
    formData.phoneNumber.trim().length >= 10 && // Basic validation
    formData.dob.trim() !== '' &&
  ...
  const getCycleText = (date: number) => {
    return `${date} to ${date} of every month`; // Simple "1 to 1" style text
  };
  ...
  return (
    <div className="w-full max-w-md bg-white sm:shadow-lg sm:rounded-xl sm:overflow-hidden h-screen sm:h-auto flex flex-col mx-auto">
      ...
      <div className="flex-1 overflow-y-auto p-6 pb-24">
        ...
        {/* Salary Cycle */}
        <div className="mb-5" onClick={() => setShowCyclePicker(true)}>
           <div className="group relative w-full rounded-t-md bg-white border border-gray-200 rounded-lg px-4 py-3 flex items-center justify-between cursor-pointer hover:border-blue-400 transition-colors">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Salary Cycle</label>
                <p className="text-sm font-semibold text-gray-900">{getCycleText(formData.salaryCycleDate)}</p>
              </div>
        ...
        {/* Salary Access */}
        <div className="mb-8">
           <div className="relative w-full rounded-lg bg-gray-50 border border-gray-200 px-4 py-3">
              <label className="block text-xs font-medium text-gray-500 mb-1">Salary Access</label>
              <select 
                name="salaryAccess"
                ...
                className="block w-full border-none bg-transparent p-0 text-gray-900 focus:ring-0 sm:text-sm font-medium outline-none appearance-none"
              >
                <option>Allow till previous cycle</option>
                <option>Allow till current date</option>
                <option>Disable access</option>
              </select>
              <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 pointer-events-none" />
           </div>
        </div>
      ...
      <CycleDateModal 
        ...
        onSelect={(date) => setFormData(prev => ({ ...prev, salaryCycleDate: date }))}
      />
